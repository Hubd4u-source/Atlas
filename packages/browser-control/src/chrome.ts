...
import { ensurePortAvailable, CONFIG_DIR } from "./utils.js";
...
import type { ResolvedBrowserConfig, ResolvedBrowserProfile } from "./types.js";
import {
...
const log = {
    info: (msg: string) => console.log(`[Browser] ${msg}`),
...
export async function launchOpenClawChrome(
    ...
    log.info(
        `🦞 browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
    );
