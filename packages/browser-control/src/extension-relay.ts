...
import { rawDataToString } from "./cdp.helpers.js";
...
export async function ensureChromeExtensionRelayServer(opts: {
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
        switch (cmd.method) {
            case "Browser.getVersion":
                return {
                    protocolVersion: "1.3",
                    product: "Chrome/atlas-Extension-Relay",
                    revision: "0",
                    userAgent: "Atlas-Extension-Relay",
                    jsVersion: "V8",
                };
    ...
    wssExtension.on("connection", (ws) => {
        ...
        ws.on("message", (data) => {
            ...
            if (parsed && parsed.method === "forwardCDPEvent") {
                broadcastToCdpClients(parsed.params);
            }
        });
    ...
    wssCdp.on("connection", (ws) => {
        ...
        ws.on("message", async (data) => {
            ...
            try {
                ...
                sendResponseToCdp(ws, { id: cmd.id, error: { message: String(err) } });
            }
        });
        ws.on("close", () => {
    ...
    return new Promise((resolve, reject) => {
        server.listen(info.port, info.host, () => {
            ...
            serversByPort.set(info.port, relay);
            resolve(relay);
        });
